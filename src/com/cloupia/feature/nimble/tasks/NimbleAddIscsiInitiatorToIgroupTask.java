...
public class NimbleAddIscsiInitiatorToIgroupTask extends AbstractTask {

	private static Logger logger = Logger.getLogger( NimbleAddIscsiInitiatorToIgroupTask.class );
	
	@Override
	public void executeCustomAction(CustomActionTriggerContext context,
			CustomActionLogger actionLogger) throws Exception {
		NimbleAddIscsiInitiatorToIgroupConfig config = (NimbleAddIscsiInitiatorToIgroupConfig) context.loadConfigObject();
		...
		context.getChangeTracker().undoableResourceAdded("assetType", "idString", "Nimble Add Initiator to Initiator Group", 
				...
				new NimbleRemoveIscsiInitiatorFromIgroupTask().getTaskName(), new NimbleRemoveIscsiInitiatorFromIgroupConfig(config));
		
        try
        {
            context.saveOutputValue(NimbleConstants.NIMBLE_INITIATOR_GROUP_NAME, config.getInitiatorGroupName());
            context.saveOutputValue(NimbleConstants.NIMBLE_INITIATOR_NAME, config.getInitiatorName());
	...
	public TaskConfigIf getTaskConfigImplementation() {
		return new NimbleAddIscsiInitiatorToIgroupConfig();
	}

	@Override
	public String getTaskName() {
		return NimbleAddIscsiInitiatorToIgroupConfig.displayLabel;
	}
	...
	public TaskOutputDefinition[] getTaskOutputDefinitions() {
		...
		ops[1] = new TaskOutputDefinition( NimbleConstants.NIMBLE_INITIATOR_NAME, NimbleConstants.GENERIC_TEXT_INPUT, "Three" );
		return ops;
	}
}
